...
int main() {
    ...
    set<string> greater{"cats", "trees"};
    set<string> fewer{"pomeranians", "goldfish"};
    ...
    for (auto possible_sue : sue_map) {
        ...
        for (auto known_attribute: possible_sue.second) {
            if (greater.count(known_attribute.first)) {

                if (possible_sue.second[known_attribute.first] <= from_sue[known_attribute.first]) {
                    discrepency = true;
                }
                
            } else if (fewer.count(known_attribute.first)) {

                if (possible_sue.second[known_attribute.first] >= from_sue[known_attribute.first]) {
                    discrepency = true;
                }
                
            } else if (from_sue[known_attribute.first] != known_attribute.second) {
                discrepency = true;
            }
        }
